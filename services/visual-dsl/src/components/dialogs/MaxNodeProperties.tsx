...
interface MaxNodePropertiesDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: MaxNodeProperties) => void;
  onDelete: () => void;
  initialProperties: MaxNodeProperties;
}

export const MaxNodePropertiesDialog: React.FC<MaxNodePropertiesDialogProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  const handleSave = () => {
    if (!canSave) {
      alert('max() requires at least one operand.');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-800 dark:border-gray-200 shadow-xl max-w-xl w-full mx-4">
        <div className="bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b border-gray-800 dark:border-gray-200 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">max() Properties</h3>
          <button
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Provide one or more expressions. The <code>max()</code> helper returns the greatest value in the list.
          </p>

          <div className="space-y-3">
            {operands.map((value, index) => (
              <div className="flex items-center gap-2" key={`max-operand-${index}`}>
