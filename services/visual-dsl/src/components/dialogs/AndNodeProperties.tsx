...
interface AndNodePropertiesDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: AndNodeProperties) => void;
  onDelete: () => void;
  initialProperties: AndNodeProperties;
}

export const AndNodePropertiesDialog: React.FC<AndNodePropertiesDialogProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  const [operands, setOperands] = React.useState<string[]>(
    Array.isArray(initialProperties.operands) && initialProperties.operands.length > 0
      ? initialProperties.operands
      : ['condA', 'condB']
  );
  ...
  const handleSave = () => {
    if (!canSave) {
      alert('Provide at least two boolean expressions.');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-800 dark:border-gray-200 shadow-xl max-w-xl w-full mx-4">
        <div className="bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b border-gray-800 dark:border-gray-200 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">and() Properties</h3>
          <button
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Provide boolean expressions to evaluate. The <code>and()</code> helper short-circuits on the first <code>false</code> value.
          </p>

          <div className="space-y-3">
            {operands.map((value, index) => (
              <div className="flex items-center gap-2" key={`and-operand-${index}`}>
