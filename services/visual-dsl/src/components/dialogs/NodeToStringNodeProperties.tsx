...
interface NodeToStringNodePropertiesProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: NodeToStringNodeProperties) => void;
  onDelete: () => void;
  initialProperties: NodeToStringNodeProperties;
}

export interface NodeToStringNodeProperties {
  node: string; // variable name to convert to string
}

export const NodeToStringNodePropertiesDialog: React.FC<NodeToStringNodePropertiesProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Converts the node into its string representation (e.g., serialized form).
            </p>
          </div>
      ...
      <div className="absolute top-1/2 left-1/2 transform translate-x-64 -translate-y-1/2 max-w-sm p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg shadow-lg">
        <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
          Returns the string form of the given node.
        </p>
