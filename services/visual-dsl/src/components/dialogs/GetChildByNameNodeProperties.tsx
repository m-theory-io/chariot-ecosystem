...
interface GetChildByNameNodePropertiesProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: GetChildByNameNodeProperties) => void;
  onDelete: () => void;
  initialProperties: GetChildByNameNodeProperties;
}

export interface GetChildByNameNodeProperties {
  node: string; // variable name of the node to search within
  name: string; // child name to get
}

export const GetChildByNameNodePropertiesDialog: React.FC<GetChildByNameNodePropertiesProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      ...
      <div className="absolute top-1/2 left-1/2 transform translate-x-64 -translate-y-1/2 max-w-sm p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg shadow-lg">
        <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
          Returns the child with the exact provided name.
        </p>
        ...
        <p className="text-xs text-blue-600 dark:text-blue-400 mt-2 italic">
          ⚠️ Backend signature errors if not found; consider guarding with findByName first if you want DBNull.
        </p>
      </div>
    </div>
  );
