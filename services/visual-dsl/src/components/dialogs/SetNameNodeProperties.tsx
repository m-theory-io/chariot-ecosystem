...
interface SetNameNodePropertiesProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: SetNameNodeProperties) => void;
  onDelete: () => void;
  initialProperties: SetNameNodeProperties;
}

export interface SetNameNodeProperties {
  variableName: string; // node variable
  name: string; // new name for node
}

export const SetNameNodePropertiesDialog: React.FC<SetNameNodePropertiesProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  useEffect(() => {
    setCanSave(variableName.trim().length > 0 && name.trim().length > 0);
  }, [variableName, name]);

  const handleSave = () => {
    if (!canSave) return;
    onSave({ variableName: variableName.trim(), name: name.trim() });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-800 dark:border-gray-200 shadow-xl max-w-md w-full mx-4">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">The variable of the node to rename.</p>
          </div>

          {/* New Name */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">New Name (required):</label>
            <Input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full" placeholder="e.g. account" />
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Enter the node's new name. Codegen will quote this.</p>
          </div>
      ...
      <div className="absolute top-1/2 left-1/2 transform translate-x-64 -translate-y-1/2 max-w-sm p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg shadow-lg">
        <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">Renames a node by setting its name.</p>
