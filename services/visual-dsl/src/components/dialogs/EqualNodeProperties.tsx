...
export interface EqualNodeProperties {
  operands: string[];
  leftOperand?: string; // legacy data
  rightOperand?: string; // legacy data
}

interface EqualNodePropertiesDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: EqualNodeProperties) => void;
  onDelete: () => void;
  initialProperties: EqualNodeProperties;
}

export const EqualNodePropertiesDialog: React.FC<EqualNodePropertiesDialogProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  const [operands, setOperands] = React.useState<string[]>(() => {
    ...
    const { leftOperand, rightOperand } = initialProperties;
    if (leftOperand || rightOperand) {
      return [leftOperand || 'valueA', rightOperand || 'valueB'];
    }
  ...
  const handleSave = () => {
    if (!canSave) {
      alert('Provide at least two expressions to compare.');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-800 dark:border-gray-200 shadow-xl max-w-xl w-full mx-4">
        <div className="bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b border-gray-800 dark:border-gray-200 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">equal() Properties</h3>
          <button
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Provide two or more expressions. The <code>equal()</code> helper returns <code>true</code> only when every operand matches the first.
          </p>

          <div className="space-y-3">
            {operands.map((value, index) => (
              <div className="flex items-center gap-2" key={`equal-operand-${index}`}>
