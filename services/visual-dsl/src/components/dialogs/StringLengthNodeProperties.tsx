...
export interface StringLengthNodeProperties {
  value: string;
}

interface StringLengthNodePropertiesDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: StringLengthNodeProperties) => void;
  onDelete: () => void;
  initialProperties: StringLengthNodeProperties;
}

export const StringLengthNodePropertiesDialog: React.FC<StringLengthNodePropertiesDialogProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  const handleSave = () => {
    if (!canSave) {
      alert('Provide the string (or expression) you want to measure.');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-800 dark:border-gray-200 shadow-xl max-w-md w-full mx-4">
        <div className="bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b border-gray-800 dark:border-gray-200 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">strlen() Properties</h3>
          <button
        ...
        <div className="p-6 space-y-5">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            The <code>strlen()</code> helper is a dispatcher alias for <code>length()</code>. Provide the string (or expression) you want to measure and it returns the number of characters.
          </p>
