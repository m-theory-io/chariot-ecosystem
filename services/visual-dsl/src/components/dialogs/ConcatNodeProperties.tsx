...
export interface ConcatNodeProperties {
  operands: string[];
}

interface ConcatNodePropertiesDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: ConcatNodeProperties) => void;
  onDelete: () => void;
  initialProperties: ConcatNodeProperties;
}

export const ConcatNodePropertiesDialog: React.FC<ConcatNodePropertiesDialogProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  const updateOperand = (index: number, nextValue: string) => {
    setOperands((prev) => prev.map((entry, idx) => (idx === index ? nextValue : entry)));
  };
  ...
  const handleSave = () => {
    if (!canSave) {
      alert('Provide at least one expression to concat.');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-800 dark:border-gray-200 shadow-xl max-w-xl w-full mx-4">
        <div className="bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b border-gray-800 dark:border-gray-200 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">concat() Properties</h3>
          <button
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Provide one or more expressions. The <code>concat()</code> helper forwards every argument to the Go runtime, so you can freely mix literals, variables, and function calls.
          </p>

          <div className="space-y-3">
            {operands.map((value, index) => (
              <div className="flex items-center gap-2" key={`concat-operand-${index}`}>
