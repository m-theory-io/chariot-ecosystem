...
interface RemoveAttributeNodePropertiesProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: RemoveAttributeNodeProperties) => void;
  onDelete: () => void;
  initialProperties: RemoveAttributeNodeProperties;
}

export interface RemoveAttributeNodeProperties {
  variableName: string; // node variable
  attributeName: string; // attribute key to remove
}

export const RemoveAttributeNodePropertiesDialog: React.FC<RemoveAttributeNodePropertiesProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  useEffect(() => {
    setCanSave(variableName.trim().length > 0 && attributeName.trim().length > 0);
  }, [variableName, attributeName]);

  const handleSave = () => {
    if (!canSave) return;
    onSave({ variableName: variableName.trim(), attributeName: attributeName.trim() });
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              The variable of the node to remove the attribute from.
            </p>
          </div>
          ...
          <div>
            ...
            <Input
              type="text"
              value={attributeName}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAttributeName(e.target.value)}
              className="w-full"
              placeholder="e.g. email"
            />
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Exact key of the attribute to remove.
            </p>
          </div>
      ...
      <div className="absolute top-1/2 left-1/2 transform translate-x-64 -translate-y-1/2 max-w-sm p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg shadow-lg">
        <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
          Removes an attribute by key from the given node.
        </p>
