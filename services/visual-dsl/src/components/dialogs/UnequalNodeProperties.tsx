...
export interface UnequalNodeProperties {
  operands: string[];
  leftOperand?: string; // legacy support
  rightOperand?: string; // legacy support
}

interface UnequalNodePropertiesDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: UnequalNodeProperties) => void;
  onDelete: () => void;
  initialProperties: UnequalNodeProperties;
}

export const UnequalNodePropertiesDialog: React.FC<UnequalNodePropertiesDialogProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  const [operands, setOperands] = React.useState<string[]>(() => {
    ...
    const left = initialProperties.leftOperand || 'valueA';
    const right = initialProperties.rightOperand || 'valueB';
    return [left, right];
  });
  ...
  const canSave = sanitizedOperands.length >= 2;

  const handleSave = () => {
    if (!canSave) {
      alert('Provide at least two operands.');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-800 dark:border-gray-200 shadow-xl max-w-xl w-full mx-4">
        <div className="bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b border-gray-800 dark:border-gray-200 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">unequal() Properties</h3>
          <button
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Provide at least two expressions. The <code>unequal()</code> helper only returns <code>true</code> when no operands share the same value.
          </p>

          <div className="space-y-3">
            {operands.map((value, index) => (
              <div className="flex items-center gap-2" key={`unequal-operand-${index}`}>
          ...
          <div className="flex justify-between items-center">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">Minimum of two operands.</p>
          </div>
