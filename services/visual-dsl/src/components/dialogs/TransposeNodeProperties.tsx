...
export interface TransposeNodeProperties {
  matrix: string;
}

interface TransposeNodePropertiesDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (properties: TransposeNodeProperties) => void;
  onDelete: () => void;
  initialProperties: TransposeNodeProperties;
}

export const TransposeNodePropertiesDialog: React.FC<TransposeNodePropertiesDialogProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  const canSave = matrix.trim().length > 0;

  const handleSave = () => {
    if (!canSave) {
      alert('Provide the matrix expression to transpose.');
      return;
    }
    onSave({ matrix: matrix.trim() });
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-800 dark:border-gray-200 shadow-xl max-w-md w-full mx-4">
        <div className="bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b border-gray-800 dark:border-gray-200 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">transpose() Properties</h3>
          <button
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Specify the matrix to transpose. The helper mirrors the runtime <code>transpose()</code> closure.
          </p>
